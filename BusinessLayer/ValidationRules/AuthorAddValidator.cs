...
{
    public class AuthorAddValidator:AbstractValidator<Author>
    {
        public readonly IEnumerable<Author> _Author;

        public AuthorAddValidator(IEnumerable<Author> author)
        {
            _Author = author;
            //AuthorName
            ...
            //AuthorMail
            RuleFor(x => x.AuthorMail).Must(IsMailUnique).WithMessage("Mail zaten kullanılıyor lütfen başka bir mail adresi kullanınız");
        ...
        public bool IsMailUnique(Author author, string newValue)
        {
            return _Author.All(x =>
              x.Equals(author.AuthorMail) || x.AuthorMail != newValue);
        }
    }
}
